...
hist_coef<-function(data,bottomlevel,toplevel,fe_data,toplevel_inds=NULL,
  ...
  m<- ggplot(df, aes(x =est,colour=toplevel,fill=toplevel))+
    ...
    panel_border()+
    xlab(paste0( "Own price elasticities"))+
    ylab("Count")+
  ...
  if (FALSE) {
    for (name.est in estimator.names){
      show(name.est)
      est<-data[,grep(name.est,colnames(data))]
      df<-data.frame(x=1:length(est),est=est,toplevel=data$toplevel_vec)
      hists[[name.est]]<-ggplot(df, aes(x =est,colour=toplevel,fill=toplevel))+
        geom_histogram()+
        xlab(paste0( "Own price elasticities"))+
        ylab("Count")+
        scale_y_continuous(limits=c(0,maxy))+
    ...
    ncols<<-dim(data)[2]
    m<-plot_grid(hists[[1]],hists[[2]],hists[[3]],ncol=length(estimator.names),
                 labels=estimator.names,label_size = 30,
