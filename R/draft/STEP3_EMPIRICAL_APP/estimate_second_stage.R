...
ss_estimates<-main(my_data=cbind(fs,my_data),
                   ...
                   xlims=xlims,figdirectory=figdirectory,all_levels=c("Level1_Name",    "Level2_Name"))
...
ss_estimates<-main(my_data=cbind(fs,my_data),
                   ...
                   xlims=xlims,figdirectory=figdirectory,all_levels=c("Level1_Name",    "Level2_Name","Level3_Name"))
## number of distinct Lasso elasticities
length(unique(ss_estimates$Lasso))
## number of positive OLS elasticities
sum(ss_estimates$OLS>0)
...
# write.table(print(xtable(ss_estimates[,c(1,2,3,7,8,11)],digits=3),include.rownames=FALSE), paste0(tabledirectory,"/",categoryname,"/Level3group.txt"))
...
if (categoryname !="Snacks") {
  ...
  ss_estimates<-main(my_data=cbind(fs,my_data),
                     ...
                     grouping_level=grouping_level,second_stage_method_names=second_stage_method_names,
                     lambda_ridge=2*log(length(subset_inds)),outname=paste0( "Level3",categoryname),
                     xlims=xlims,figdirectory=figdirectory,all_levels=c("Level1_Name",    "Level2_Name","Level3_Name","Level4_Name"))
  ## number of distinct Lasso elasticities
  length(unique(ss_estimates$Lasso))
  ## number of positive OLS elasticities
  sum(ss_estimates$OLS>0)
  ...
  # write.table(print(xtable(ss_estimates[,c(1,2,3,8,9,12)],digits=3),include.rownames=FALSE), paste0(tabledirectory,"/",categoryname,"/Level4group.txt"))
