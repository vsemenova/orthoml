...
for (categoryname in categorynames) {
  ...
  first_stage_price_formula<-as.formula(paste0("logpr~(",paste(c("week","month"),collapse="+"),
                                               ")","+(Level1+Level2+Level3)*(",
                                               paste(c("logprice_lag","logprice_lag_2"),collapse="+"),
                                               ")"))
  first_stage_sales_formula<-as.formula(paste0("logsls~(",paste(c("week","month"),collapse="+"),
                                               ")","+(Level1+Level2+Level3)*(",
